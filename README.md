...
```
### JSON Mode
**JSON Mode** provides info in **JSON** format, ready to be exported.

**/etc example with JSON Mode enabled (latest revision in version 1.3):**
```
{
  "tool": "inspect",
  "tool_version": "1.3",
...
```
### Line Mode
**Line Mode** shows all info you might need in a single, horizontal line, while keeping all the necessary info:

**/etc example with Line Mode enabled (on version 1.3):**

```/etc  symlink  11B  rwxr-xr-x   root (0)  wheel (0)  2024-07-17 13:49```
...
|  --line  |  -l  |  1.3 and up  |  Show info in one line  |
